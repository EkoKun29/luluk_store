...
@endsection
@section('create-link')
    <a href="{{ route('receivable-payments.cash.create') }}" class="btn btn-primary">Tambah</a>
@endsection
@section('table')
    <table class="table table-striped" style="width: 100%;" id="tableReceivableCash">
        <thead>
            <tr>
                <th>
                    <h6>Tanggal</h6>
        ...
        <tbody>
            @foreach ($receivablePayments as $receivablePayment)
                <tr>
                    ...
                    <td class="min-width">
                        <p>{{ $receivablePayment->sale->consumer }}</p>
                    </td>
                    <td class="min-width">
...
@push('js')
    <script>
        $(document).ready(function() {
            let table = new DataTable('#tableReceivableCash');
        })
    </script>
@endpush
