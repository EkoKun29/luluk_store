...
@endsection
@section('create-link')
    <a href="{{ route('purchases.debt.create') }}" class="btn btn-primary">Tambah</a>
@endsection
@section('table')
    <table class="table table-striped" id="tableDebtPurchase" style="width: 100%;">
        <thead>
            <tr>
                <th>
                    <h6>#</h6>
        ...
        <tbody>
            @foreach ($purchases as $purchase)
                <tr>
                    ...
                    <td class="min-width">
                        <p>{{ 'Rp ' . number_format($purchase->total - ($purchase->amount_paid + $purchase->debt_paid_off), 2, ',', '.') }}</p>
                    </td>
                    <td>
                        <div class="action">
...
@push('js')
    <script>
        $(document).ready(function() {
            let table = new DataTable('#tableDebtPurchase', {
                "columnDefs": [{
                    "targets": 6,
                    "orderable": false
                }]
